...
def linsok(lista, key):
    """Funktion för linjärsökning. Tar in lista med artistobjekt samt eftersökt artistnamn som str. O(n)"""
    found = False                       # Default
...
def binary_search(lista, key):
    """Funktion för binär sökninng. Börjar från mitten, stänger sedan in successivt. O(log2(n)"""
    #lista.sort()
    low = 0                             # Initierar low
...
def hashsearch(dicti, key):
    """Funktion för hashsökning. Tar in en dictionary (hashlista) samt eftersök namn som type str. O(1)"""
...
def main():
    ...
    # Tidtagning
    print(binary_search(lista, testartist))
    input("Nästa")
    sok_tid = timeit.timeit(stmt = lambda: binary_search(lista, testartist), number = 100000)
    print("Sökningen tog", round(sok_tid, 10)/100000, "sekunder")
